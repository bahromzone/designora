...
def upgrade() -> None:
    ...
    # ── ASSIGNMENTS ──
    conn.execute(
        sa.text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS course_id  INTEGER")
    )
    conn.execute(
        sa.text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS title      VARCHAR")
    )
    conn.execute(
        sa.text(
    ...
    conn.execute(
        sa.text(
            "ALTER TABLE assignments ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ"
        )
    )
    ...
    # ── CERTIFICATES ──
    conn.execute(
        sa.text("ALTER TABLE certificates ADD COLUMN IF NOT EXISTS course_id  INTEGER")
    )
    conn.execute(
        sa.text(
    ...
    conn.execute(
        sa.text("ALTER TABLE certificates ADD COLUMN IF NOT EXISTS title      VARCHAR")
    )
    ...
    # ── COURSES ──
    conn.execute(
        sa.text("ALTER TABLE courses ADD COLUMN IF NOT EXISTS thumbnail_url VARCHAR")
    )
    conn.execute(
        sa.text("ALTER TABLE courses ADD COLUMN IF NOT EXISTS instructor_id INTEGER")
    )
