...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("course_ids", sa.JSON()),
        sa.Column("is_active", sa.Boolean(), server_default=sa.false()),
        sa.Column("readiness_note", sa.Text()),
    ...
    op.create_table(
        ...
        sa.Column("used_seats", sa.Integer(), server_default="0"),
        sa.Column("status", sa.String()),
        sa.Column(
            "created_at", sa.DateTime(timezone=True), server_default=sa.func.now()
        ),
    ...
    op.create_table(
        "team_license_members",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column(
            "license_id",
            sa.Integer(),
            sa.ForeignKey("team_licenses.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.Column("status", sa.String()),
        sa.UniqueConstraint("license_id", "email", name="uq_team_license_member_email"),
    )
    op.create_table(
        ...
        sa.Column("requested_installments", sa.Integer()),
        sa.Column("status", sa.String()),
