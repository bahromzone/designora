...
for router in (profile.router, admin_courses.router, admin_dashboard.router, analytics.router, assignments.router, assignments_upload.router, blog.router, calendar.router, certificates.router, checkout_experience.router, course_builder.router, course_forum.router, courses_api.router, discovery.router, forum.router, gamification.router, gamification_v2.router, google.router, instructor.router, instructor_analytics.router, instructors.router, learning.router, learning_paths.router, media.router, moderation.router, monetization.router, notes.router, notifications.router, offline_sync.router, pages.router, payments.router, portfolio.router, privacy.router, qa.router, quiz.router, referrals.router, reviews.router, seo.router, support.router, system.router, token.router, uploads.router, users.router, public_router, auth.router):
...
def admin_list_users(db: Session = Depends(get_db), admin: User = Depends(_require_admin)):
    return [{"id": user.id, "name": user.name, "email": user.email, "role": user.role, "is_active": user.is_active} for user in db.query(User).order_by(User.id.desc()).all()]
