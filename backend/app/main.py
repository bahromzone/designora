...
app.add_middleware(SecurityHeadersMiddleware);app.add_middleware(RequestLoggingMiddleware);app.add_middleware(IPBlockingMiddleware)
...
for api_router in (profile.router,admin_courses.router,analytics.router,assignments.router,assignments_upload.router,blog.router,calendar.router,certificates.router,checkout_experience.router,course_builder.router,courses_api.router,discovery.router,forum.router,gamification.router,google.router,instructor.router,instructor_analytics.router,instructors.router,learning.router,learning_paths.router,media.router,notes.router,notifications.router,pages.router,payments.router,portfolio.router,privacy.router,qa.router,quiz.router,referrals.router,reviews.router,system.router,token.router,uploads.router,users.router,public_router,auth.router):app.include_router(api_router)
...
def admin_list_users(db:Session=Depends(get_db),admin:User=Depends(_require_admin)):return [{"id":r.id,"name":r.name,"email":r.email,"role":r.role,"is_active":r.is_active} for r in db.query(User).order_by(User.id.desc()).all()]
