...
Admin va superadmin rollari uchun.
...
def _wrong_users_route():
    raise HTTPException(status_code=410, detail="Bu endpoint ko'chirildi. /api/admin/users ishlatilsin.")
...
def list_courses(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    return [
        {"id": c.id, "title": c.title, "category": c.category, "price": c.price,
         "is_active": c.is_active, "thumbnail_url": c.thumbnail_url,
...
def create_course(data: CourseCreate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    try:
        ...
        db.rollback(); raise HTTPException(status_code=500, detail=f"Saqlashda xatolik: {exc}")
...
def update_course(course_id: int, data: CourseUpdate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    try:
        ...
        db.rollback(); raise HTTPException(status_code=500, detail=f"Yangilashda xatolik: {exc}")
...
def delete_course(course_id: int, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    try:
        ...
        db.rollback(); raise HTTPException(status_code=500, detail=f"O'chirishda xatolik: {exc}")
...
def toggle_course(course_id: int, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    db.commit()
    return JSONResponse({"message": "Kurs faolligi yangilandi", "is_active": course.is_active})
