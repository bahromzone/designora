...
def create_course(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Saqlashda xatolik: {exc}")
    return JSONResponse(
        status_code=201,
        content={
...
def update_course(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Yangilashda xatolik: {exc}")
    return {"message": "Kurs yangilandi", "id": course.id}


@router.delete("/{course_id}")
def delete_course(
    course_id: int, db: Session = Depends(get_db), admin: User = Depends(require_admin)
):
    course = db.query(Course).filter(Course.id == course_id).first()
    if not course:
        raise HTTPException(status_code=404, detail="Kurs topilmadi")
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"O'chirishda xatolik: {exc}")
