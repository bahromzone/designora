...
def coupons(db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    return [{"id": row.id, "code": row.code, "type": row.type, "value": row.value, "max_uses": row.max_uses, "used_count": row.used_count, "is_active": row.is_active, "expires_at": row.expires_at.isoformat() if row.expires_at else None} for row in rows]
