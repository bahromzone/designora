...
def _event(row, timezone):
 zone = _zone(timezone)
 start = _utc(row["starts_at"]).astimezone(zone)
 end = _utc(row["ends_at"]).astimezone(zone) if row.get("ends_at") else None
...
def events(start: datetime, end: datetime, timezone: str = "UTC", event_type: str | None = None, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
 user = _user(db, email); _zone(timezone)
 ...
 enrolled_ids = [row[0] for row in db.query(Enrollment.course_id).filter(Enrollment.user_id == user.id).all()]
 rows = []
 if enrolled_ids and (not event_type or event_type == "assignment"):
  assignments = db.query(Assignment).filter(Assignment.course_id.in_(enrolled_ids), Assignment.due_date >= start_utc, Assignment.due_date < end_utc).all()
  rows.extend({"id": f"assignment-{item.id}", "title": item.title or "Topshiriq", "event_type": "assignment", "starts_at": item.due_date, "ends_at": None, "course_id": item.course_id, "link": f"/organish/{item.course_id}", "description": "Assignment deadline"} for item in assignments)
 ...
 for item in query.order_by(CalendarEvent.starts_at.asc()).all():
  rows.append({"id": item.id, "title": item.title, "event_type": item.event_type, "starts_at": item.starts_at, "ends_at": item.ends_at, "course_id": item.course_id, "link": item.link, "description": item.description})
...
def create_event(data: EventIn, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
 ...
 item = CalendarEvent(user_id=user.id, title=data.title, event_type=data.event_type, starts_at=starts_at, ends_at=ends_at, course_id=data.course_id, link=data.link, description=data.description)
 db.add(item); db.commit(); db.refresh(item)
 return {"id": item.id, "message": "Calendar event yaratildi"}


@router.delete("/events/{event_id}")
def delete_event(event_id: int, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
 user = _user(db, email)
 item = db.query(CalendarEvent).filter(CalendarEvent.id == event_id, CalendarEvent.user_id == user.id).first()
