...
def global_search(
    ...
    if "lesson" in requested:
        rows = (db.query(Lesson, Course).join(Course, Lesson.course_id == Course.id).filter(Course.is_active == True).limit(500).all())
        groups["lesson"] = _rank(
            q,
            [{"type": "lesson", "id": lesson.id, "title": lesson.title, "subtitle": course.title, "description": lesson.description, "meta": lesson.type or "Dars", "image_url": course.thumbnail_url, "url": f"/organish/{course.id}?lesson={lesson.id}"} for lesson, course in rows],
            limit,
        )

    if "instructor" in requested:
        rows = (db.query(User).filter(User.is_active == True, User.role.in_(["instructor", "admin", "superadmin"])).limit(200).all())
        groups["instructor"] = _rank(q, [{"type": "instructor", "id": row.id, "title": row.name or "Designora instruktor", "subtitle": row.bio, "description": row.bio, "meta": row.location or "Instruktor", "image_url": row.avatar_url, "url": f"/instruktor/{row.id}"} for row in rows], limit)

    if "blog" in requested:
        rows = (db.query(BlogPost).filter(BlogPost.is_published == True).order_by(BlogPost.published_at.desc()).limit(300).all())
        groups["blog"] = _rank(q, [{"type": "blog", "id": row.id, "title": row.title, "subtitle": row.excerpt, "description": row.excerpt or row.meta_description, "meta": row.tags or "Maqola", "image_url": row.cover_image_url, "url": f"/blog/{row.slug}"} for row in rows], limit)

    if "forum" in requested:
        rows = db.query(ForumThread).order_by(ForumThread.updated_at.desc()).limit(500).all()
        groups["forum"] = _rank(q, [{"type": "forum", "id": row.id, "title": row.title, "subtitle": (row.body or "")[:180], "description": (row.body or "")[:300], "meta": f"{row.category or 'Forum'} · {row.views or 0} ko‘rish", "image_url": None, "url": f"/forum/{row.id}"} for row in rows], limit)

    total = sum(len(items) for items in groups.values())
    return {"query": q, "total": total, "groups": groups, "suggestion": None if total else "Kengroq yoki boshqa kalit so‘z kiriting"}


@router.get("/search")
def search(q: str | None = None, category: str | None = None, level: str | None = None, language: str | None = None, min_price: int | None = None, max_price: int | None = None, min_rating: float | None = None, sort: str = "newest", page: int = Query(1, ge=1), per_page: int = Query(12, ge=1, le=50), db: Session = Depends(get_db)):
    query = db.query(Course).filter(Course.is_active == True)
    if q:
        like = f"%{q.strip()}%"
        query = query.filter(or_(Course.title.ilike(like), Course.subtitle.ilike(like), Course.description.ilike(like)))
    if category: query = query.filter(Course.category == category.lower())
    if level: query = query.filter(Course.level == level)
    if language: query = query.filter(Course.language == language)
    if min_price is not None: query = query.filter(Course.price >= min_price)
    if max_price is not None: query = query.filter(Course.price <= max_price)
    if min_rating is not None: query = query.filter(Course.rating_avg >= min_rating)
    column, descending = _SORTABLE.get(sort, _SORTABLE["newest"])
    query = query.order_by(column.desc() if descending else column.asc())
    total = query.count()
    items = query.offset((page - 1) * per_page).limit(per_page).all()
    return {"total": total, "page": page, "per_page": per_page, "pages": (total + per_page - 1) // per_page, "results": [_card(course) for course in items]}
...
def categories(db: Session = Depends(get_db)):
    ...
    for (category,) in rows:
        if category: counts[category] = counts.get(category, 0) + 1
