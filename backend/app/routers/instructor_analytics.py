...
def _course_ids(db: Session, user: User) -> list[int]:
    return [
        row.id for row in db.query(Course).filter(Course.instructor_id == user.id).all()
    ]
...
def instructor_analytics(
    ...
    orders = (
        db.query(Order).filter(Order.course_id.in_(ids), Order.status == "paid").all()
    )
    events = db.query(AnalyticsEvent).all()
    views = sum(
        1
        for event in events
        if event.name in {"course_view", "course_viewed"}
        and (event.props or {}).get("course_id") in ids
    )
    funnel = [
        {"step": "views", "count": views, "conversion": 100.0 if views else 0.0},
        {
            ...
            "conversion": _pct(len(enrollments), views),
        },
        {
            "step": "paid",
            "count": len(orders),
            "conversion": _pct(len(orders), len(enrollments)),
        },
        {
            "step": "completed",
            "count": sum(
                1 for row in enrollments if (row.progress_percent or 0) >= 100
            ),
            "conversion": _pct(
                sum(1 for row in enrollments if (row.progress_percent or 0) >= 100),
                len(enrollments),
            ),
        },
    ]
    ...
    for lesson in lessons:
        progress = (
            db.query(LessonProgress).filter(LessonProgress.lesson_id == lesson.id).all()
        )
        lesson_rows.append(
            {
                "lesson_id": lesson.id,
                ...
                "completion_rate": _pct(
                    sum(1 for row in progress if row.is_completed), len(progress)
                ),
            }
        )
    ...
    for quiz in quizzes:
        ...
        average = (
            round(sum(row.score or 0 for row in attempts) / len(attempts), 1)
            if attempts
            else 0.0
        )
        quiz_rows.append(
            {
                ...
                "pass_rate": _pct(
                    sum(1 for row in attempts if row.passed), len(attempts)
                ),
                "difficulty": (
                    "hard" if average < 60 else "medium" if average < 80 else "easy"
                ),
            }
        )
    ...
    for marker in (25, 50, 75, 100):
        ...
        count = sum(
            1
            for event in events
            if event.name in names and (event.props or {}).get("course_id") in ids
        )
        dropoff.append({"percent": marker, "viewers": count})

    assignment_ids = [
        row.id
        for row in db.query(Assignment).filter(Assignment.course_id.in_(ids)).all()
    ]
    submissions = (
    ...
    assignment_metrics = {
        ...
        "average_grade": round(
            sum(row.grade or 0 for row in submissions if row.grade is not None)
            / max(1, sum(1 for row in submissions if row.grade is not None)),
            1,
        ),
    }
    ...
    buckets = Counter(
        "positive" if row.rating >= 4 else "neutral" if row.rating == 3 else "negative"
        for row in reviews
    )
    sentiment = {
        ...
        "average_rating": (
            round(sum(row.rating for row in reviews) / len(reviews), 2)
            if reviews
            else 0.0
        ),
    }
...
def export_csv(
    ...
    for row in payload["lessons"]:
        writer.writerow(
            ["lesson", row["title"], row["completion_rate"], row["completed"]]
        )
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": "attachment; filename=instructor-analytics.csv"
        },
    )
