...
def require_admin(user: User = Depends(current_user)) -> User:
    if not user.is_active or user.role not in _ADMIN_ROLES:
...
def review_application(application_id: int, data: ReviewIn, db: Session = Depends(get_db), reviewer: User = Depends(require_admin)):
    ...
    application.status = data.status; application.reviewed_by_id = reviewer.id; application.reviewed_at = datetime.now(UTC)
    if data.status == "approved" and user:
        user.name = application.name; user.bio = application.bio; user.website = application.portfolio_url; user.role = "instructor"
    db.commit()
