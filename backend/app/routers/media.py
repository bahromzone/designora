...
def sign_lesson_video(lesson_id: int, ttl: int = Query(3600, ge=60, le=86400), email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    primary = video_service.build_signed_url(sources[0]["url"], settings.media_signing_key, base_url=settings.MEDIA_CDN_BASE_URL, ttl_seconds=ttl)
    return {"lesson_id": lesson.id, **primary, "sources": signed_sources, "subtitles": lesson.subtitles or [], "resume_seconds": progress.position_seconds if progress else 0}
