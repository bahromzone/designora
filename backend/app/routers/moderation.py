...
def report(
    data: ReportIn, user: User = Depends(current_user), db: Session = Depends(get_db)
):
...
def queue(
    status: str = "open", db: Session = Depends(get_db), _: User = Depends(admin)
):
    rows = (
...
def review(
    ...
    reviewer: User = Depends(admin),
):
    if data.status not in {"resolved", "dismissed"}:
        raise HTTPException(
            status_code=400, detail="Status resolved yoki dismissed bo'lishi kerak"
        )
