...
def lesson_notes(
    ...
    user = _user(db, email)
    query = db.query(LessonNote).filter(
        LessonNote.lesson_id == lesson_id, LessonNote.user_id == user.id
    )
    if q:
    ...
    return [
        ...
        for row in query.order_by(
            LessonNote.timestamp_seconds.asc(), LessonNote.id.asc()
        ).all()
    ]
...
def my_notes(
    ...
    if q:
        query = query.join(Lesson).filter(
            or_(
                LessonNote.body.ilike(f"%{q.strip()}%"),
                Lesson.title.ilike(f"%{q.strip()}%"),
            )
        )
    if course_id:
    ...
    return [
        _note(row)
        for row in query.order_by(LessonNote.updated_at.desc()).limit(limit).all()
    ]
...
def export_notes(
    ...
    for row in rows:
        ...
        if format == "markdown":
            output.write(
                f"## {row['lesson_title'] or 'Dars'} ({stamp})\n\n{row['body']}\n\n"
            )
        else:
            output.write(
                f"{row['lesson_title'] or 'Dars'} [{stamp}]\n{row['body']}\n\n"
    ...
    return PlainTextResponse(
        ...
        media_type=media,
        headers={
            "Content-Disposition": f'attachment; filename="designora-notes.{"md" if format == "markdown" else "txt"}"'
        },
    )
...
def delete_note(
    note_id: int, email: str = Depends(get_current_user), db: Session = Depends(get_db)
):
    user = _user(db, email)
    row = (
        db.query(LessonNote)
...
def bookmarks(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    user = _user(db, email)
    rows = (
        db.query(LessonBookmark, Lesson)
        .join(Lesson, LessonBookmark.lesson_id == Lesson.id)
        .filter(LessonBookmark.user_id == user.id, LessonBookmark.is_bookmarked == True)
        .order_by(LessonBookmark.updated_at.desc())
        .all()
    )
    return [
        {
...
def set_bookmark(
    ...
    row = (
        ...
        .filter(
            LessonBookmark.user_id == user.id, LessonBookmark.lesson_id == lesson.id
        )
        .first()
    )
    if not row:
