...
def _preference(db: Session, user_id: int) -> ReminderPreference:
    row = db.query(ReminderPreference).filter(ReminderPreference.user_id == user_id).first()
    if not row:
        row = ReminderPreference(user_id=user_id)
        db.add(row)
        db.commit()
        db.refresh(row)
    return row
...
def _preference_dict(row: ReminderPreference, subscriptions: int = 0) -> dict:
    return {
        key: getattr(row, key)
        for key in (
            "email_enabled", "in_app_enabled", "push_enabled", "lesson_reminders",
            "deadline_reminders", "review_reminders", "marketing_enabled", "frequency",
    ...
    def valid_timezone(cls, value):
        if value:
            try:
                ...
                raise ValueError("Noto‘g‘ri timezone") from error
        return value


class PushSubscriptionIn(BaseModel):
    endpoint: str = Field(min_length=10, max_length=1000)
    p256dh: str | None = Field(default=None, max_length=300)
    auth: str | None = Field(default=None, max_length=300)
...
def unread_count(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    user = _get_user(db, email)
    return {"unread": db.query(Notification).filter(Notification.user_id == user.id, Notification.is_read == False).count()}
...
def get_preferences(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return _preference_dict(_preference(db, user.id), count)
...
def update_preferences(data: PreferencePatch, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    return _preference_dict(row, db.query(PushSubscription).filter(PushSubscription.user_id == user.id).count())
...
def subscribe_push(data: PushSubscriptionIn, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not row:
        row = PushSubscription(user_id=user.id, **data.model_dump())
        db.add(row)
    else:
        row.user_id, row.p256dh, row.auth = user.id, data.p256dh, data.auth
    preference = _preference(db, user.id)
    preference.push_enabled = True
    db.commit()
    return {"message": "Browser push yoqildi", "id": row.id}


@router.delete("/push-subscriptions")
def unsubscribe_push(endpoint: str, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    user = _get_user(db, email)
    removed = db.query(PushSubscription).filter(PushSubscription.user_id == user.id, PushSubscription.endpoint == endpoint).delete()
    if db.query(PushSubscription).filter(PushSubscription.user_id == user.id).count() == 0:
        _preference(db, user.id).push_enabled = False
    db.commit()
...
def test_reminder(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    user = _get_user(db, email)
    preference = _preference(db, user.id)
    local_now = datetime.now(ZoneInfo(preference.timezone))
    current = local_now.strftime("%H:%M")
    quiet = (preference.quiet_start <= current or current < preference.quiet_end) if preference.quiet_start > preference.quiet_end else preference.quiet_start <= current < preference.quiet_end
    channels = []
    if not quiet:
        if preference.in_app_enabled:
            db.add(Notification(user_id=user.id, message="Test reminder muvaffaqiyatli ishladi", type="reminder", link="/calendar"))
            channels.append("in_app")
        if preference.email_enabled:
            channels.append("email")
        if preference.push_enabled and db.query(PushSubscription).filter(PushSubscription.user_id == user.id).count():
...
def mark_read(notification_id: int, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    row.is_read = True
    db.commit()
    return {"message": "O'qilgan deb belgilandi", "id": row.id}


@router.post("/read-all")
def mark_all_read(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    user = _get_user(db, email)
    updated = db.query(Notification).filter(Notification.user_id == user.id, Notification.is_read == False).update({Notification.is_read: True})
    db.commit()
