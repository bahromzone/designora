...
def eligible(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    rows = (
        ...
        .filter(AssignmentSubmission.user_id == user.id, AssignmentSubmission.status == "graded")
...
def create(data: ProjectIn, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if data.submission_id:
        submission = db.query(AssignmentSubmission).filter(AssignmentSubmission.id == data.submission_id, AssignmentSubmission.user_id == user.id, AssignmentSubmission.status == "graded").first()
        if not submission:
    ...
    project = PortfolioProject(
        ...
        position=db.query(PortfolioProject).filter(PortfolioProject.user_id == user.id).count(),
    )
    db.add(project)
    db.commit()
    db.refresh(project)
