...
def update_profile(
    ...
    return JSONResponse({"message": "Profil muvaffaqiyatli yangilandi", "name": user.name, "avatar_url": user.avatar_url})
...
def change_password(data: ChangePasswordRequest, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if user.provider != "local":
        raise HTTPException(status_code=400, detail=f"Siz {user.provider} orqali kirganingiz uchun parolni bu yerda o'zgartira olmaysiz")
...
def get_stats(email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    user = _get_user_or_unauthorized(db, email)
    try:
        progress_rows = db.query(Progress, Course).join(Course, Progress.course_id == Course.id).filter(Progress.user_id == user.id).all()
    except Exception:
        db.rollback()
        progress_rows = []
    ...
    pending_assignments = db.query(Assignment).filter(Assignment.user_id == user.id, Assignment.is_completed == False).count()
    courses_list = []
    for prog, course in progress_rows:
        courses_list.append({
            "id": course.id, "title": course.title, "category": course.category or "general",
            "progress": prog.percent, "is_completed": prog.percent >= 100,
    ...
    return {"courses_enrolled": courses_enrolled, "courses_completed": courses_completed, "hours_learned": hours_learned, "certificates": certificates_count, "pending_assignments": pending_assignments, "points": user.points or 0, "streak_days": user.streak_days or 0, "level": user.level or 1, "courses": courses_list, "activity": activity}
...
def update_progress(course_id: int, data: ProgressUpdateRequest, email: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if progress.percent >= 100:
        ...
        if not existing_cert:
            db.add(Certificate(user_id=user.id, course_id=course_id, title=f"{course.title} sertifikati", issued_at=datetime.now(UTC)))
