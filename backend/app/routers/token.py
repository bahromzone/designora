"""Token Router, refresh-token rotation and inactive-account revocation."""
...
def _is_production()->bool: return settings.ENVIRONMENT=="production"
...
def _revoke_all(db:Session,user_id:int)->int:
    return db.query(RefreshToken).filter(RefreshToken.user_id==user_id,RefreshToken.revoked_at.is_(None)).update({RefreshToken.revoked_at:datetime.now(UTC)})

def issue_refresh_token(db:Session,user:User,*,user_agent:str|None=None)->str:
    raw=token_service.generate_refresh_token()
    db.add(RefreshToken(user_id=user.id,token_hash=token_service.hash_token(raw),expires_at=token_service.refresh_expiry(),user_agent=(user_agent or "")[:255] or None))
    db.flush();return raw
...
def refresh(request:Request,response:Response,db:Session=Depends(get_db)):
    ...
    if not rec.is_active:
        _revoke_all(db,rec.user_id);db.commit()
        raise HTTPException(status_code=401,detail="Refresh-token qayta ishlatildi — barcha sessiyalar bekor qilindi")
    user=db.query(User).filter(User.id==rec.user_id).first()
    if not user: raise HTTPException(status_code=401,detail="Foydalanuvchi topilmadi")
    if not user.is_active:
        _revoke_all(db,user.id);db.commit()
        raise HTTPException(status_code=403,detail=INACTIVE_ACCOUNT_DETAIL)
    new_raw=issue_refresh_token(db,user,user_agent=request.headers.get("user-agent"))
    rec.revoked_at=datetime.now(UTC);rec.replaced_by=token_service.hash_token(new_raw);db.commit()
    access=create_access_token(user.email)
    response.set_cookie(key="access_token",value=access,httponly=True,secure=_is_production(),max_age=3600,samesite="strict")
...
def logout_all(request:Request,response:Response,email:str=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    revoked=_revoke_all(db,user.id);db.commit();response.delete_cookie("access_token");response.delete_cookie(_REFRESH_COOKIE,path="/api/auth")
...
def issue_refresh_for_current(request:Request,response:Response,email:str=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    raw=issue_refresh_token(db,user,user_agent=request.headers.get("user-agent"));db.commit();_set_refresh_cookie(response,raw)
