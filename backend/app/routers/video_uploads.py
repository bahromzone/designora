...
def _client():
    if not all(
        (
            ...
            settings.VIDEO_STORAGE_PUBLIC_BASE_URL,
        )
    ):
        raise HTTPException(
            status_code=503, detail="Video storage productionda sozlanmagan"
        )
    return boto3.client(
        "s3",
...
def initiate(
    ...
    if data.size > max_bytes:
        raise HTTPException(
            status_code=413,
            detail=f"Video {settings.VIDEO_UPLOAD_MAX_GB} GB dan katta bo'lmasligi kerak",
        )
    ext = _extension(data.filename)
    if ext not in _ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=400, detail="MP4, WebM, MOV yoki M4V kerak")
    ...
    result = client.create_multipart_upload(
        ...
        Metadata={"course-id": str(course_id), "lesson-id": str(lesson_id)},
    )
    ...
    urls = [
        client.generate_presigned_url(
            ...
            ExpiresIn=settings.VIDEO_UPLOAD_URL_TTL_SECONDS,
        )
    ...
    return {
        ...
        "key": key,
        "part_size": part_size,
        "parts": [{"part_number": i + 1, "url": url} for i, url in enumerate(urls)],
        "expires_in": settings.VIDEO_UPLOAD_URL_TTL_SECONDS,
    }
...
def complete(
    ...
    if not _key_for(course_id, lesson_id, data.key):
        raise HTTPException(status_code=400, detail="Upload key noto'g'ri")
    parts = sorted(data.parts, key=lambda item: item.part_number)
    if len({item.part_number for item in parts}) != len(parts) or any(
        not item.etag.strip() for item in parts
    ):
        raise HTTPException(
            status_code=400, detail="Upload qismlari yoki ETag noto'g'ri"
        )
    client = _client()
    ...
    lesson.video_sources = [
        {
            "label": "Original",
            "url": public_url,
            "type": _MIME_BY_EXT.get(ext, "video/mp4"),
        }
    ]
...
def abort(
    ...
    _lesson(course_id, lesson_id, db, user)
    if not _key_for(course_id, lesson_id, key):
        raise HTTPException(status_code=400, detail="Upload key noto'g'ri")
    _client().abort_multipart_upload(
        Bucket=settings.VIDEO_STORAGE_BUCKET, Key=key, UploadId=upload_id
    )
