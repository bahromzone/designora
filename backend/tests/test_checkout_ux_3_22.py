...
def setup(db):
    ...
    course = Course(
        title="Checkout UX", price=100000, is_active=True, status="published"
    )
    db.add_all(
        [user, course, Coupon(code="UX20", type="percent", value=20, is_active=True)]
    )
    db.commit()
    return user, course
...
def test_quote_safe_checkout_deduplicates_and_retries(client, db_session):
    ...
    first = client.post(
        "/api/payments/checkout-safe", json=body, headers=headers(user.email)
    )
    second = client.post(
        "/api/payments/checkout-safe", json=body, headers=headers(user.email)
    )
    ...
    db_session.commit()
    retried = client.post(
        f"/api/payments/orders/{order.id}/retry", headers=headers(user.email)
    )
