...
def test_roadmap_filters_duration_instructor_and_sort(client, db_session):
    mentor = User(
        email="filter-mentor@example.com",
        name="Filter Mentor",
        role="instructor",
        is_active=True,
    )
    ...
    response = client.get(
        ...
        params={"instructor_id": mentor.id, "min_duration": 60, "max_duration": 180},
    )
    assert response.status_code == 200
    assert [item["title"] for item in response.json()["results"]] == ["Short"]

    sorted_response = client.get(
        "/api/discovery/search", params={"sort": "duration_asc"}
    )
    assert [item["title"] for item in sorted_response.json()["results"]] == [
        "Short",
        "Other",
        "Long",
    ]
