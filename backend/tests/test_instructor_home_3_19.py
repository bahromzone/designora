...
def test_instructor_home_returns_operational_queues_and_health(client, db_session):
    ...
    enrollment = Enrollment(
        ...
        course_id=course.id,
        progress_percent=10,
        enrolled_at=datetime.now(UTC) - timedelta(days=20),
    )
    ...
    db_session.add(
        LessonQuestion(
            ...
            course_id=course.id,
            user_id=student.id,
            body="Briefni qanday topshiraman?",
        )
    )
    db_session.commit()
    ...
    response = client.get(
        ...
        headers=_auth("teacher319@example.com"),
    )
...
def test_instructor_home_is_scoped_to_owned_courses(client, db_session):
    ...
    db_session.add_all(
        [
            Course(title="Owner course", instructor_id=owner.id),
            Course(title="Other course", instructor_id=other.id),
        ]
    )
    db_session.commit()
    ...
    response = client.get(
        "/api/analytics/instructor",
        headers=_auth("owner319@example.com"),
    )
    assert response.status_code == 200
