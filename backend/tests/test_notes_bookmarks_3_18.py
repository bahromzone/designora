...
def test_note_search_recent_export_and_bookmark(client, db_session):
    ...
    created = client.post(
        f"/api/notes/lessons/{lesson.id}",
        headers=headers,
        json={"body": "Contrast hierarchy", "timestamp_seconds": 73},
    )
    ...
    assert (
        client.get(
            ...
            headers=headers,
            params={"q": "hierarchy"},
        ).json()[0]["timestamp_seconds"]
        == 73
    )
    assert (
        client.get("/api/notes/recent", headers=headers).json()["lesson_title"]
        == "Typography"
    )
    assert "Typography (1:13)" in client.get("/api/notes/export", headers=headers).text
    assert (
        client.put(f"/api/notes/bookmarks/{lesson.id}", headers=headers).json()[
            "bookmarked"
        ]
        is True
    )
    assert (
        client.get("/api/notes/bookmarks", headers=headers).json()[0]["lesson_id"]
        == lesson.id
    )
    assert (
        client.put(
            f"/api/notes/bookmarks/{lesson.id}",
            headers=headers,
