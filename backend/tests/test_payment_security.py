...
Qamrab olingan:
- Production konfiguratsiyasi to'lov kalitlarisiz ko'tarilmasligi kerak.
- PAYME_KEY / CLICK_SECRET_KEY bo'sh bo'lganda webhook'lar OCHIQ qolmasligi kerak.
- Bekor qilingan (refund) to'lov kursga kirishni ham yopishi kerak.
- Buzuq order_id 500 emas, tartibli xato kodi qaytarishi kerak.

Eslatma: `is_active` (bloklangan hisob) tekshiruvi allaqachon `main` dagi
`app/core/security.py` ichida va o'z testlari bilan qoplangan — bu yerda
takrorlanmaydi.
"""
...
# ── 2. Kalit sozlanmaganda webhook yopiq ────────────────────────────────────
...
def test_click_prepare_rejected_when_secret_not_configured(
    ...
    forged = hashlib.md5(raw.encode()).hexdigest()  # noqa: S324
...
# ── 3. Buzuq kirish ma'lumoti 500 bermasin ──────────────────────────────────
