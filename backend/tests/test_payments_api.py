...
def _make_course(db, price=100000, active=True):
    course = Course(
        title="Pullik kurs", price=price, is_active=active, status="published"
    )
    db.add(course)
    db.commit()
    db.refresh(course)
...
def test_checkout_requires_auth(client, db_session):
    ...
    resp = client.post(
        "/api/payments/checkout", json={"course_id": course.id, "provider": "payme"}
    )
...
def test_payme_full_flow_grants_access(client, db_session, monkeypatch):
    ...
    resp = client.post(
        ...
        json=_rpc(
            ...
            {"amount": amount_tiyin, "account": {"order_id": order.id}},
        ),
        headers=_payme_auth(),
    )
    ...
    resp = client.post(
        ...
        json=_rpc(
            "CreateTransaction",
            {"id": "tx-1", "amount": amount_tiyin, "account": {"order_id": order.id}},
        ),
        headers=_payme_auth(),
    )
...
def test_payme_perform_is_idempotent(client, db_session, monkeypatch):
    ...
    client.post(
        ...
        json=_rpc(
            "CreateTransaction",
            {"id": "tx-9", "amount": amount_tiyin, "account": {"order_id": order.id}},
        ),
        headers=h,
    )
    client.post(
        "/api/payments/payme",
        json=_rpc("PerformTransaction", {"id": "tx-9"}),
        headers=h,
    )
...
def test_payme_cancel_transaction(client, db_session, monkeypatch):
    ...
    client.post(
        ...
        json=_rpc(
            "CreateTransaction",
            {"id": "tx-x", "amount": amount_tiyin, "account": {"order_id": order.id}},
        ),
        headers=h,
    )
    resp = client.post(
        "/api/payments/payme", json=_rpc("CancelTransaction", {"id": "tx-x"}), headers=h
    )
    assert resp.json()["result"]["state"] == -1
