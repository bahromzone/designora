      ...
      <div className="relative flex flex-col gap-6 w-full overflow-hidden pb-10">
          ...
          {[...REVIEWS, ...REVIEWS].map((review, i) => (
            ...
            >
              <div className="flex items-center gap-4 mb-5">
                <img src={review.image} alt={review.name} className="w-14 h-14 rounded-full object-cover shadow-sm group-hover:ring-2 ring-purple-400 ring-offset-2 transition-all" />
                <div>
              ...
              <p className="text-slate-600 leading-relaxed text-[15px]">"{review.text}"</p>
            </div>
          ))}
        </motion.div>
        ...
        >
          {[...REVIEWS, ...REVIEWS].reverse().map((review, i) => (
            ...
            >
              <div className="flex items-center gap-4 mb-5">
                <img src={review.image} alt={review.name} className="w-14 h-14 rounded-full object-cover shadow-sm group-hover:ring-2 ring-indigo-400 ring-offset-2 transition-all" />
                <div>
                  <h4 className="font-bold text-slate-900">{review.name}</h4>
                  <p className="text-xs text-slate-500 font-medium">{review.role}</p>
                </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 mt-20">
        <div className="grid lg:grid-cols-5 gap-12 items-center">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6 relative">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-2 relative z-10">Karyera o'sishi</h3>
            ...
            {SUCCESS_STORIES.map((story, i) => (
              ...
              >
                <div className="flex-1">
                  <p className="text-xs font-bold text-slate-400 mb-1">{story.name}</p>
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-semibold text-slate-500 line-through decoration-slate-300">{story.before}</span>
                    <svg className="w-4 h-4 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    <span className="text-sm font-bold text-slate-900">{story.after}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 sm:flex-col sm:items-end sm:gap-2">
                  ...
                  <p className="text-xs font-bold text-slate-400">{story.company}</p>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 w-full p-8 md:p-12">
              ...
              <p className="text-white/70 text-sm md:text-base max-w-lg">
                Designora platformasida ta'lim olgan Jasurning shaxsiy hikoyasini tomosha qiling va o'z karyerangizni boshlash uchun ilhom oling.
              </p>
            </div>
          </motion.div>
