    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs font-semibold tracking-widest uppercase text-muted">Kurslar</p>
            <p className="font-serif text-lg font-semibold text-ink mt-0.5">Har darajaga mos</p>
          </div>
          {/* Toggle pill */}
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="flex -space-x-2">
            ...
            <div className="h-7 w-7 rounded-full border-2 border-white bg-surface flex items-center justify-center text-[9px] font-bold text-muted">+12</div>
          </div>
        ...
        <div className="rounded-xl bg-surface p-3 mb-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-xs font-bold text-ink">$7,548 <span className="text-muted font-normal">/ $24,000</span></p>
          </div>
        ...
        <div className="flex items-center gap-2 rounded-full bg-amber/10 px-3 py-1.5 mb-4 w-fit">
          ...
          <p className="text-xs font-semibold text-amber">Baholash testlari — 99%</p>
        </div>

        {/* Weekly recommendations */}
        <div className="rounded-xl border border-border p-3">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold text-ink">Haftalik tavsiyalar</p>
            <div className="h-5 w-5 rounded bg-amber flex items-center justify-center text-white text-[10px] font-bold">A</div>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <p className="text-xs font-semibold text-ink tabular-nums">{pct}%</p>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        <div className="h-7 w-7 rounded-full bg-ink flex items-center justify-center">
          <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 text-white" fill="none" stroke="currentColor" strokeWidth="1.8">
...
export default function Hero({ isAuthenticated }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(subRef.current,   { opacity: 0, y: 18 }, { opacity: 1, y: 0, duration: 0.5 }, "-=0.35");
      tl.fromTo(btnsRef.current,  { opacity: 0, y: 14 }, { opacity: 1, y: 0, duration: 0.45 }, "-=0.3");
      ...
      countersRef.current.forEach((el, i) => {
        ...
        gsap.fromTo(el, { innerText: 0 }, {
          innerText: s.num, duration: 1.8, delay: 0.7 + i * 0.1,
          ease: "power2.out",
          snap: isFloat ? { innerText: 0.1 } : { innerText: 1 },
          onUpdate() {
            const v = parseFloat(el.innerText);
            el.innerText = (isFloat ? v.toFixed(1) : Math.round(v)) + s.suffix;
          },
        });
      });
    }, rootRef);
    ...
    <section ref={rootRef} className="shell pt-12 pb-20 sm:pt-20 sm:pb-28">
      <div className="grid gap-12 lg:grid-cols-[1fr_auto] lg:items-center lg:gap-16">
        ...
        <div className="max-w-2xl">
          ...
          {/* Sub */}
          <p ref={subRef} className="mt-6 max-w-lg text-base leading-8 opacity-0" style={{ color: "var(--ink-60)" }}>
            Kapsula kolleksiya, styling va vizual sotuv jarayonlarini bir ritmda o'rgatadi.
            Interfeys yengil, tajriba premium — natija portfolioga tayyor.
          </p>

          {/* Buttons */}
          <div ref={btnsRef} className="mt-8 flex flex-wrap items-center gap-3 opacity-0">
            <Link
          ...
          <div ref={statsRef} className="mt-10 grid grid-cols-3 gap-4 max-w-lg">
            {STATS.map((s, i) => (
              <div key={s.label} className="card rounded-2xl px-4 py-4 hover:-translate-y-0.5 transition-transform">
