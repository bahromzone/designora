...
export default function NotificationBell() {
  ...
  // wrapRef bor edi, lekin hech qayerda ishlatilmasdi: dropdown tashqariga
  // bosilganda yopilmasdi va ekranda osilib qolardi. Escape ham qo'shildi.
  useEffect(() => {
  ...
  async function toggle() {
    ...
    if (next) {
      ...
      try {
        ...
        // Backend hozir massiv qaytaradi, lekin paginatsiyaga o'tsa ham qulamaydi.
  ...
  async function readOne(item) {
    ...
    try {
      ...
      // Avval faqat hisoblagich kamayardi, ro'yxatdagi element o'qilmagan
        ...
        {settings ? <ReminderSettings onClose={() => setSettings(false)} /> : <div className="w-[360px] max-w-[calc(100vw-2rem)]">
          <header className="flex items-center justify-between gap-2 border-b px-4 py-3">
            ...
            <span className="flex items-center gap-3">
              {unread > 0 && <button onClick={readAll} className="text-xs font-semibold text-violet-600 hover:underline">Barchasini o‘qildi</button>}
              <button onClick={() => setSettings(true)} aria-label="Reminder sozlamalari">⚙ Sozlamalar</button>
            </span>
          </header>
