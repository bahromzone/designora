...
function readCompleted() {
  try {
    return localStorage.getItem(ONBOARDING_KEYS.completed) === "1";
  } catch {
    return true;
  }
}
...
function Icon({ name }) {
  const paths = {
    ...
    spark: (
      <>
        <path d="m12 3-1.2 4.1L7 9l3.8 1.9L12 15l1.2-4.1L17 9l-3.8-1.9Z" />
        <path d="m5 15-.7 2.3L2 18.5l2.3 1.2L5 22l.7-2.3L8 18.5l-2.3-1.2Z" />
      </>
    ),
  };
...
export default function OnboardingModal() {
  ...
  const weeklySessions = Math.max(
    2,
    Math.min(5, Math.round(profile.weeklyHours / 1.5))
  );
  const sessionMinutes =
    Math.round((profile.weeklyHours * 60) / weeklySessions / 5) * 5;
        ...
        <div className="onboarding-layout">
          <aside className="onboarding-aside">
            <div>
              ...
              <p>
                Javoblaringiz asosida kurs tavsiyalari va haftalik rejangiz
                moslanadi.
              </p>
            </div>
            <ol>
            ...
            <div className="onboarding-step" key={step}>
              {step === 0 && (
                <>
                  ...
                  <h1 id="onboarding-title">
                    Design sizni qayerga olib borishi kerak?
                  </h1>
                  <p className="onboarding-lead">
                    Eng muhim maqsadni tanlang. Tavsiya yo‘lingiz shunga qarab
                    tuziladi.
                  </p>
                  <div className="onboarding-options">
                    {GOALS.map((option) => (
                      <button
                        ...
                        type="button"
                        className={
                          profile.goal === option.id ? "is-selected" : ""
                        }
                        onClick={() => update("goal", option.id)}
                      >
                        <span>
                        ...
                        <i>
                          {profile.goal === option.id && <Icon name="check" />}
                        </i>
                      </button>
                    ))}
                  </div>
              ...
              {step === 1 && (
                <>
                  ...
                  <p className="onboarding-lead">
                    Uchtagacha tanlang. Fokusni yo‘qotmaymiz.
                  </p>
                  <div className="interest-cloud">
                    {INTERESTS.map((item) => (
                      <button
                        ...
                        className={
                          profile.interests.includes(item) ? "is-selected" : ""
                        }
                  ...
                  <p className="selection-count">
                    {profile.interests.length}/3 tanlandi
                  </p>
                </>
              )}
              {step === 2 && (
                <>
                  ...
                  <p className="onboarding-lead">
                    Darajangiz kontent murakkabligi va boshlash nuqtasini
                    belgilaydi.
                  </p>
                  <div className="onboarding-options level-options">
                    {LEVELS.map((option) => (
                      <button
                        ...
                        type="button"
                        className={
                          profile.level === option.id ? "is-selected" : ""
                        }
                        onClick={() => update("level", option.id)}
                      >
                        <span>
                          <strong>{option.title}</strong>
                          <small>{option.note}</small>
                        ...
                        <i>
                          {profile.level === option.id && <Icon name="check" />}
                        </i>
                      </button>
                    ))}
                  </div>
              ...
              {step === 3 && (
                <>
                  ...
                  <p className="onboarding-lead">
                    Katta va’da emas, bajariladigan reja. Haftasiga qancha vaqt
                    ajratasiz?
                  </p>
                  <div className="hours-row">
                    {HOURS.map((hours) => (
                      <button
                        ...
                        type="button"
                        className={
                          profile.weeklyHours === hours ? "is-selected" : ""
                        }
                        onClick={() => update("weeklyHours", hours)}
                      >
                        <strong>{hours}</strong>
                        <small>soat</small>
                      </button>
                  ...
                  <div className="plan-preview">
                    ...
                    <div>
                      <small>Sizning boshlang‘ich rejangiz</small>
                      <strong>
                        Haftasiga {weeklySessions} marta, taxminan{" "}
                        {sessionMinutes} daqiqadan
                      </strong>
                      <p>
                        {profile.interests[0] || "Tanlangan yo‘nalish"} bo‘yicha
                        izchil progress uchun yetarli temp.
                      </p>
                    </div>
                  </div>
                  <label className="reminder-toggle">
                    <span>
                      <Icon name="clock" />
                      <i>
                        <strong>O‘qish reminderi</strong>
                        <small>Rejadan ortda qolsangiz eslatamiz</small>
                      </i>
                    </span>
                    <input
                      type="checkbox"
