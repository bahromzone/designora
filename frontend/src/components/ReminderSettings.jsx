...
const TOGGLES = [
  ["email_enabled", "Email"],
  ["in_app_enabled", "Ilova ichida"],
  ["push_enabled", "Browser push"],
  ["lesson_reminders", "Dars eslatmalari"],
  ["deadline_reminders", "Deadline eslatmalari"],
  ["review_reminders", "Review sanalari"],
  ["marketing_enabled", "Yangilik va takliflar"],
];
...
export default function ReminderSettings({ onClose }) {
  ...
  const save = async (patch) => {
    ...
    setValue(optimistic);
    try {
      setValue(await reminderApi.update(patch, token));
      setStatus("Saqlandi");
    } catch (error) {
      setStatus(error.message);
      reminderApi.get(token).then(setValue);
    }
  };
  if (!value)
    return (
      <div className="reminder-settings">
        <p>{status || "Sozlamalar yuklanmoqda..."}</p>
      </div>
    );
  return (
    <section className="reminder-settings" aria-labelledby="reminder-title">
      <header>
        <div>
          <small>Reminderlar</small>
          <h2 id="reminder-title">Bildirishnoma sozlamalari</h2>
        </div>
      ...
      <div className="reminder-row">
        <label>
          ...
          >
            <option value="instant">Darhol</option>
            <option value="daily">Kunlik jamlama</option>
      ...
      <div className="reminder-row">
        <label>
          Quiet hours boshlanishi
          <input
            type="time"
            value={value.quiet_start}
            onChange={(event) => save({ quiet_start: event.target.value })}
          />
        </label>
        <label>
          Tugashi
          <input
            ...
            onChange={(event) => save({ quiet_end: event.target.value })}
          />
        </label>
      </div>
      <footer>
            ...
            reminderApi
              ...
              .then((result) =>
                setStatus(
                  result.quiet_hours
                    ? "Quiet hours faol"
                    : `${result.channels.join(", ")} orqali test yuborildi`
                )
              )
          }
        >
