...
export default function VideoPlayer({
  ...
  useEffect(() => {
    if (!lessonId) {
      setManifest(
        src
          ? {
              sources: [{ label: "Auto", url: src, type: "video/mp4" }],
              subtitles: [],
            }
          : null
      );
  ...
  useEffect(() => {
    ...
    const onMetadata = () => {
      const saved =
        manifest.resume_seconds ?? Number(localStorage.getItem(storageKey));
      if (shouldResume(saved, video.duration))
  ...
  if (error)
    return (
      <div className="video-player-state" role="alert">
        <b>Video ochilmadi</b>
        <span>{error}</span>
  ...
  if (!manifest?.sources?.length)
    return (
      <div className="video-player-state">Bu dars videosi mavjud emas.</div>
    );
  ...
  return (
    ...
    >
      <video
        ...
        onError={() =>
          setError(
            "Media manbasi javob bermadi. Boshqa sifatni tanlang yoki qayta urining."
          )
        }
      >
        ...
        <label>
          ...
          >
            {manifest.sources.map((source, index) => (
              <option value={index} key={`${source.label}-${source.url}`}>
                {source.label}
              </option>
            ))}
          </select>
