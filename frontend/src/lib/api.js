...
export const learningApi = {
  enroll: (courseId) => request(`/api/learning/enroll/${courseId}`, { method: "POST" }),
  unenroll: (courseId) => request(`/api/learning/enroll/${courseId}`, { method: "DELETE" }),
  myCourses: () => request("/api/learning/my-courses"),
  learn: (courseId) => request(`/api/learning/courses/${courseId}`),
  completeLesson: (lessonId) => request(`/api/learning/lessons/${lessonId}/complete`, { method: "POST" }),
  uncompleteLesson: (lessonId) => request(`/api/learning/lessons/${lessonId}/uncomplete`, { method: "POST" }),
};

export const discoveryApi = {
  search: (params = {}) => request(withQuery("/api/discovery/search", params)),
  categories: () => request("/api/discovery/categories"),
  bestselling: (limit = 6) => request(withQuery("/api/discovery/recommendations/bestselling", { limit })),
  similar: (courseId, limit = 6) => request(withQuery(`/api/discovery/recommendations/similar/${courseId}`, { limit })),
};

export const quizApi = {
  courseQuizzes: (courseId) => request(`/api/quiz/courses/${courseId}/quizzes`),
  take: (quizId) => request(`/api/quiz/quizzes/${quizId}`),
  submit: (quizId, answers) => request(`/api/quiz/quizzes/${quizId}/submit`, { method: "POST", body: JSON.stringify({ answers }) }),
};

export const reviewsApi = {
  summary: (courseId) => request(`/api/reviews/courses/${courseId}/summary`),
  list: (courseId) => request(`/api/reviews/courses/${courseId}`),
  upsert: (courseId, body) => request(`/api/reviews/courses/${courseId}`, { method: "POST", body: JSON.stringify(body) }),
  remove: (reviewId) => request(`/api/reviews/${reviewId}`, { method: "DELETE" }),
};

export const qaApi = {
  list: (lessonId) => request(`/api/qa/lessons/${lessonId}/questions`),
  ask: (lessonId, body) => request(`/api/qa/lessons/${lessonId}/questions`, { method: "POST", body: JSON.stringify(body) }),
  answer: (questionId, body) => request(`/api/qa/questions/${questionId}/answers`, { method: "POST", body: JSON.stringify(body) }),
  resolve: (questionId) => request(`/api/qa/questions/${questionId}/resolve`, { method: "PATCH" }),
};

export const notesApi = {
  forLesson: (lessonId) => request(`/api/notes/lessons/${lessonId}`),
  create: (lessonId, body) => request(`/api/notes/lessons/${lessonId}`, { method: "POST", body: JSON.stringify(body) }),
  update: (noteId, body) => request(`/api/notes/${noteId}`, { method: "PATCH", body: JSON.stringify(body) }),
  remove: (noteId) => request(`/api/notes/${noteId}`, { method: "DELETE" }),
};

export const certificatesApi = {
  mine: () => request("/api/certificates/my"),
  issue: (courseId) => request(`/api/certificates/courses/${courseId}/issue`, { method: "POST" }),
  download: (certId) => request(`/api/certificates/${certId}/download`),
  verify: (code) => request(`/api/certificates/verify/${code}`),
};

export const mediaApi = {
  signLesson: (lessonId) => request(`/api/media/lessons/${lessonId}/sign`, { method: "POST" }),
};

export const paymentsApi = {
  checkout: (body) => request("/api/payments/checkout", { method: "POST", body: JSON.stringify(body) }),
...
export const forumApi = {
  ...
  createThread: (body) => request("/api/forum/threads", { method: "POST", body: JSON.stringify(body) }),
  reply: (threadId, body) => request(`/api/forum/threads/${threadId}/posts`, { method: "POST", body: JSON.stringify(body) }),
};
...
export const instructorsApi = {
  ...
  courses: (instructorId) => request(`/api/instructors/${instructorId}/courses`),
};

export const gamificationApi = {
  me: () => request("/api/gamification/me"),
  badges: () => request("/api/gamification/badges"),
  leaderboard: (limit = 20) => request(withQuery("/api/gamification/leaderboard", { limit })),
};

export const analyticsApi = {
  instructor: () => request("/api/analytics/instructor"),
  admin: () => request("/api/analytics/admin"),
  track: (body) => request("/api/analytics/track", { method: "POST", body: JSON.stringify(body) }),
};

export const instructorApi = {
  listCourses: () => request("/api/instructor/courses"),
  getCourse: (courseId) => request(`/api/instructor/courses/${courseId}`),
  createCourse: (body) => request("/api/instructor/courses", { method: "POST", body: JSON.stringify(body) }),
  updateCourse: (courseId, body) => request(`/api/instructor/courses/${courseId}`, { method: "PATCH", body: JSON.stringify(body) }),
  publishCourse: (courseId) => request(`/api/instructor/courses/${courseId}/publish`, { method: "POST" }),
  unpublishCourse: (courseId) => request(`/api/instructor/courses/${courseId}/unpublish`, { method: "POST" }),
  createModule: (courseId, body) => request(`/api/instructor/courses/${courseId}/modules`, { method: "POST", body: JSON.stringify(body) }),
  updateModule: (moduleId, body) => request(`/api/instructor/modules/${moduleId}`, { method: "PATCH", body: JSON.stringify(body) }),
  deleteModule: (moduleId) => request(`/api/instructor/modules/${moduleId}`, { method: "DELETE" }),
  createLesson: (courseId, body) => request(`/api/instructor/courses/${courseId}/lessons`, { method: "POST", body: JSON.stringify(body) }),
  updateLesson: (lessonId, body) => request(`/api/instructor/lessons/${lessonId}`, { method: "PATCH", body: JSON.stringify(body) }),
  deleteLesson: (lessonId) => request(`/api/instructor/lessons/${lessonId}`, { method: "DELETE" }),
};
