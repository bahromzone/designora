...
it("returns stable path statuses", () => {
  expect(pathStatus({ started: false, progress_percent: 0 })).toBe(
    "not-started"
  );
  expect(pathStatus({ started: true, progress_percent: 40 })).toBe(
    "in-progress"
  );
  expect(pathStatus({ started: true, progress_percent: 100 })).toBe(
    "completed"
  );
});
