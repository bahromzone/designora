...
export default function CertificatesPage() {
  ...
  return (
    <section className="shell py-16 sm:py-20">
      ...
      >
        Kurslarni yakunlaganingiz sari verifikatsiya qilinadigan
        sertifikatlaringiz shu yerda jamlanadi.
      </p>
      {error && (
      ...
      {items?.length === 0 && (
        ...
        >
          <p className="font-semibold text-ink">Hali sertifikat yo‘q</p>
          <p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>
            Kursni 100% tugating va yakuniy testlardan o‘ting.
          </p>
        </div>
      )}
      <div className="mt-10 grid gap-5 md:grid-cols-2">
        {items?.map((item) => (
          <article
            ...
            style={{ borderColor: "var(--border)" }}
          >
            <p className="label">{item.grade || "Bitirildi"}</p>
