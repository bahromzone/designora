...
export default function CheckoutResultPage() {
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      if (!active || TERMINAL.includes(status)) return;
  ...
  async function retry() {
    ...
    try {
      ...
      else setError("To‘lov havolasi olinmadi.");
    } catch (err) {
      setError(err.message);
    } finally {
      setBusy(false);
  ...
  return (
    <main className="checkout-shell">
      <section className="checkout-card">
        ...
        <p>
          {error || data?.failure_reason || "Provayder tasdig‘ini kutyapmiz."}
        </p>
        {status === "paid" && (
          <>
        ...
        {receipt && (
          <div>
            ...
            <p>
              To‘langan:{" "}
              {Number(receipt.paid_amount || 0).toLocaleString("uz-UZ")}{" "}
              {receipt.currency || "UZS"}
            </p>
