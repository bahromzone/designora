    ...
    <section className="shell py-16 sm:py-20">
      <header className="max-w-3xl">
        ...
        <p className="mt-3 text-ink-60">
          Kategoriya, daraja, davomiylik, narx, til, reyting va instruktor
          bo‘yicha aniq kursni toping.
        </p>
      </header>
      ...
      >
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <input
            ...
            placeholder="Kurs nomi yoki kalit so‘z"
          ...
          <select
            ...
            onChange={(event) => setFilter("instructor_id", event.target.value)}
          ...
          <input
            ...
            onChange={(event) => setFilter("min_duration", event.target.value)}
            placeholder="Min daqiqa"
            aria-label="Minimal davomiylik"
          />
          <input
            className="input"
            type="number"
            ...
            onChange={(event) => setFilter("max_duration", event.target.value)}
            placeholder="Max daqiqa"
            aria-label="Maksimal davomiylik"
          />
          <select
            className="input"
          ...
          <label className="input flex items-center gap-2">
            <input
              ...
              onChange={(event) =>
                setFilter("certificate", event.target.checked ? "true" : "")
              }
            />{" "}
            Sertifikat mavjud
          </label>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <span className="text-sm text-ink-60">
            {loading ? "Qidirilmoqda..." : `${data?.total || 0} ta kurs`}{" "}
            {activeCount ? `· ${activeCount} filtr` : ""}
          </span>
          {activeCount > 0 && (
      ...
      {loading ? (
        ...
        <div className="mt-8 rounded-2xl border p-8" role="alert">
          {error}
          <button
            className="btn-outline ml-3"
            onClick={() => setFilter("page", String(filters.page))}
          >
            Qayta urinish
          </button>
        </div>
            ...
            >
              {Array.from({ length: data.pages }, (_, index) => index + 1).map(
                (item) => (
                  <button
                    className={
                      item === filters.page ? "btn-dark" : "btn-outline"
                    }
                    key={item}
        ...
        <div className="mt-8 rounded-2xl border p-10 text-center">
          <h2 className="font-serif text-2xl">Hech narsa topilmadi</h2>
          <p className="mt-2 text-ink-60">
            Filtrlarni yumshating yoki tozalang.
          </p>
          <button className="btn-dark mt-5" onClick={reset}>
            Filtrlarni tozalash
          </button>
        </div>
      )}
