...
export function buildDashboardMetrics(courses, assignments, now = new Date()) {
  ...
  const feedback = assignments.filter(
    (item) => item.my_submission?.status === "graded"
  );
  ...
  return (
    <section className="dashboard-insights" aria-labelledby="today-plan-title">
      ...
      <div className="insights-kpis">
        ...
        <article className={metrics.overdue.length ? "danger" : ""}>
          <strong>{metrics.overdue.length}</strong>
          <span>kechikkan</span>
        </article>
        <article>
          <strong>{metrics.dueSoon.length}</strong>
          <span>3 kunda tugaydi</span>
        </article>
        <article>
          <strong>{metrics.feedback.length}</strong>
          <span>yangi feedback</span>
        </article>
        <article>
      ...
      <div className="today-list">
        ...
        {!metrics.open.length && (
          <p className="insights-empty">Bugungi ochiq vazifalar tugadi.</p>
        )}
      </div>
    </section>
  );
