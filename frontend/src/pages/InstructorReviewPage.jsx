...
export default function InstructorReviewPage() {
  ...
  return (
    <section className="review-shell">
      <header>
        <div>
          <p className="label">Instructor review</p>
          <h1>Topshiriqni baholash</h1>
        </div>
        <Link to="/instruktor-panel">Panelga qaytish</Link>
      </header>
      <div className="review-layout">
        ...
        <main>
          {!selected ? (
            <p>Tekshiriladigan ish yo‘q.</p>
          ) : (
            <>
              <article className="submission-copy">
                <p>{selected.content || "Izoh yozilmagan"}</p>
                ...
                <div
                  ...
                  onClick={(event) =>
                    ...
                    setAnnotations((items) => [
                      ...items,
                      createImageAnnotation(event, annotationNote),
                    ])
                  }
                >
              ...
              <label>
                Rasm annotation izohi
                <input
                  value={annotationNote}
                  onChange={(event) => setAnnotationNote(event.target.value)}
                  placeholder="Rasm ustiga bosishdan oldin izoh yozing"
                />
              </label>
              <ol className="review-notes">
                {annotations.map((item, index) => (
                  <li key={index}>
                    <b>{index + 1}.</b> {item.note}
                    <button
                      onClick={() =>
                        setAnnotations((rows) =>
              ...
              <div className="video-feedback">
                ...
                <button onClick={addVideoFeedback}>Qo‘shish</button>
              </div>
              <ol className="review-notes">
                {videoFeedback.map((item, index) => (
                  <li key={index}>
                    <b>
              ...
              <div className="review-form">
                ...
                <label>
                  ...
                  >
                    ...
                    <option value="rework">Qayta ishlash kerak</option>
                  </select>
                </label>
