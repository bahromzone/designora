...
function CertificatePanel({ certificate, progress, quizzes, issuing, error, onIssue }) {
  ...
  return <section className="certificate-panel" aria-live="polite"><div><p className="certificate-eyebrow">Kurs yakunlandi</p><h2>Sertifikat olishga tayyormisiz?</h2><p>{quizzes.length ? `Sertifikat uchun ${quizzes.length} ta faol testdan ham o'tish kerak.` : "Barcha darslar tugadi. Sertifikatingizni hozir oling."}</p></div><button className="btn-primary" type="button" onClick={onIssue} disabled={issuing}>{issuing ? "Tayyorlanmoqda..." : "Sertifikatni olish"}</button>{error && <p className="certificate-error" role="alert">{error}</p>}</section>;
}
...
export default function LearnPage() {
  ...
  async function issueCertificate() { setIssuing(true); setCertificateError(""); try { setCertificate(await certificatesApi.issue(courseId, token)); } catch (reason) { setCertificateError(reason.message); } finally { setIssuing(false); } }
