...
export default function PaymentHistoryPage() {
  ...
  return (
    <section className="shell py-16 sm:py-20">
      <p className="label">Hisob-kitoblar</p>
      ...
      >
        Buyurtmalar, chegirmalar va to‘lov holati bir joyda.
      </p>
      {error && (
      ...
      {items?.length === 0 && (
        ...
        >
          <p className="font-semibold text-ink">Hali buyurtmalar yo‘q</p>
          <p className="mt-2 text-sm" style={{ color: "var(--muted)" }}>
            Kurs tanlaganingizdan keyin tarix shu yerda ko‘rinadi.
          </p>
        </div>
      )}
      <div
            ...
            >
              <div>
                <p className="font-semibold text-ink">
                  {item.course_title || `Buyurtma #${item.id}`}
                </p>
                <p className="mt-1 text-sm" style={{ color: "var(--muted)" }}>
                  {date(item.created_at)} · {item.provider || "—"}
                </p>
              </div>
              <div className="text-right">
                ...
                <p
                  className={`mt-1 text-xs font-bold ${item.status === "paid" ? "text-emerald-600" : item.status === "cancelled" ? "text-red-600" : "text-amber-600"}`}
                >
                  {item.status}
                </p>
              </div>
