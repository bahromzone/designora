...
export default function PortfolioPage() {
  ...
  const active = useMemo(
    () => data?.projects.find((project) => project.id === activeId) || null,
    [data, activeId]
  );
  ...
  async function remove() {
    if (!active || !window.confirm("Bu loyihani portfolio'dan o'chirasizmi?"))
      return;
    try {
    ...
    <section className="portfolio-page">
      <header className="portfolio-header">
        <div>
          ...
          <span>
            Baholangan loyihalarni tanlang, hikoyasini yozing va ulashing.
          </span>
        </div>
        {data?.user_id && (
          <Link to={`/portfolio/u/${data.user_id}`} target="_blank">
            Public ko‘rinish ↗
          </Link>
        )}
      </header>
      ...
      {!projects.length ? (
        <div className="portfolio-empty">
          <span>01</span>
          <h2>Hali portfolio loyihasi yo‘q.</h2>
          <p>
            Baholangan topshiriq yonidagi “Portfolio’ga qo‘shish” tugmasini
            bosing.
          </p>
          <Link to="/kurslarim">Kurslarga qaytish</Link>
        </div>
      ) : (
        <div className="portfolio-builder">
          <aside className="portfolio-list">
            <div>
              <strong>{projects.length} loyiha</strong>
              <small>
                {projects.filter((item) => item.is_public).length} tasi public
              </small>
            </div>
            {projects.map((project, index) => (
              ...
              >
                ...
                <i>
                  ...
                  <small>{project.is_public ? "Public" : "Qoralama"}</small>
                </i>
              </button>
            ))}
          </aside>
          ...
          {draft && (
            <main className="portfolio-editor">
              ...
              >
                <span>
                  {draft.cover_url ? "Cover preview" : "Cover URL kiriting"}
                </span>
              </div>
              <div className="portfolio-fields">
                ...
                <label>
                  ...
                  <textarea
                    ...
                    rows={3}
                    maxLength={500}
                    placeholder="Muammo, yechim va natijani qisqa yozing"
                  />
                </label>
                <label>
                  ...
                  <textarea
                    ...
                    rows={8}
                    maxLength={8000}
                    placeholder="Jarayon, qarorlar, sinovlar va o‘rganganlaringiz..."
                  />
                </label>
                <div className="portfolio-two">
                  ...
                  <label>
                    <span>Loyiha URL</span>
                    <input
                      type="url"
                      value={draft.project_url}
                      onChange={(e) =>
                        setDraft({ ...draft, project_url: e.target.value })
                      }
                      placeholder="Figma, Behance, Drive..."
                    />
                  </label>
                </div>
                <div className="portfolio-two">
                ...
                <label className="portfolio-publish">
                  <span>
                    <strong>Public portfolio’da ko‘rsatish</strong>
                    <small>Havolaga ega odamlar loyihani ko‘ra oladi</small>
                  </span>
                  <input
                    type="checkbox"
              ...
              <footer>
                <button className="portfolio-delete" onClick={remove}>
                  O‘chirish
                </button>
                <button
