...
export default function SettingsPage() {
  ...
  return (
    <section className="shell py-16 sm:py-20">
      ...
      >
        Kirish xavfsizligini shu yerdan boshqaring. Ism, avatar va shaxsiy
        ma’lumotlar{" "}
        <Link
      ...
      <form
        ...
        className="mt-10 max-w-2xl rounded-2xl border bg-white p-6"
        style={{ borderColor: "var(--border)" }}
      >
        <p className="label">Kirish xavfsizligi</p>
        <p className="mt-3 text-sm" style={{ color: "var(--muted)" }}>
          {user?.provider && user.provider !== "local"
            ? `${user.provider} orqali kirgansiz, parol bu yerda boshqarilmaydi.`
            : "Yangi parol kamida 8 belgi, katta harf va raqamdan iborat bo‘lsin."}
        </p>
        ...
        <button
          ...
          disabled={changing || (user?.provider && user.provider !== "local")}
        >
